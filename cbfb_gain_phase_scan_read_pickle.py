...
for run in range(N_runs_fb):
    ...
    dipole_exc_quad_sideband_mag[run, :] = data['width_mode_amp']
...
for mode in plot_modes:
    ...
    for i in range(quad_unique_gains.shape[0]):
        ...
        plt.semilogy(quad_fb_phases[plot_indices], quad_exc_pos_amp[plot_indices, mode] /\
                 base_quad_pos_mode_amp[mode], label='Gain = ' + str(quad_unique_gains[i]))
    plt.legend(loc=0, fontsize='medium')
    plt.xlabel('CBFB phase [rad]')
    plt.ylabel('Relative oscillation amplitude')
